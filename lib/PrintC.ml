...
and p_declaration (qs, spec, inline, stor, { maybe_unused; target }, decl_and_inits) =
  let target = match target with | None -> empty | Some "" -> empty | Some x -> string "KRML_ATTRIBUTE_TARGET" ^^ parens (dquotes (string x)) ^^ break1 in
  let inline =
    match inline with
    | None -> empty
    | Some C11.Inline -> string "inline" ^^ space
    | Some NoInline -> string "KRML_NOINLINE" ^^ space
    | Some MustInline when target = empty -> string "inline" ^^ space
