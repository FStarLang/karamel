...
let inline_type_abbrevs ?(just_auto_generated=false) files =
  let is_kept lid =
    List.exists (fun p -> Bundle.pattern_matches_lid p lid) !Options.no_inline_type_abbrev
  in
  let map = Helpers.build_map files (fun map -> function
    ...
    | DType (lid, _, _, _, Abbrev t) when not just_auto_generated && is_kept lid ->
        (* Record the body so passes that need to look through it (e.g.
         * [Helpers.flatten_arrow] when type-checking applications) can. *)
        Hashtbl.replace Helpers.preserved_type_abbrevs lid t
    | _ -> ()
  ) in
  ...
  (* Apply the same inliner to the bodies of preserved type abbreviations so
   * that nested abbreviations (e.g. app_ctxt within error_handler) get
   * resolved when other passes look through them via
   * [Helpers.preserved_type_abbrevs]. *)
  if not just_auto_generated then
    Hashtbl.filter_map_inplace
      (fun _ t -> Some (i#visit_typ () t))
      Helpers.preserved_type_abbrevs;
