...
let rec p_type_spec = function
  ...
  | Union (name, decls) ->
      ...
      braces_with_nesting (separate_map hardline (fun p -> group (p_declaration_and_initialization p ^^ semi)) decls)
  | Struct (name, decls) ->
      group (string "struct" ^/^
      (match name with Some name -> string name | None -> empty)) ^^
      (match decls with
      | Some decls ->
          break1 ^^ braces_with_nesting (separate_map hardline (fun p -> group (p_declaration_and_initialization p ^^ semi)) decls)
      | None ->
          empty)
  | Enum (name, tags) ->
      group (string "enum" ^/^
...
and p_declaration (spec, stor, decl_and_inits) =
  ...
  separate_map (comma ^^ break 1) p_decl decl_and_inits

and p_initialization (_, _, decl_and_inits) =
  separate_map (comma ^^ break 1) p_decl_and_init decl_and_inits

and p_declaration_and_initialization (spec, stor, decl_and_inits) =
  let stor = match stor with Some stor -> p_storage_spec stor ^^ space | None -> empty in
  stor ^^ group (p_type_spec spec) ^/^
  separate_map (comma ^^ break 1) p_decl_and_init decl_and_inits
...
let rec p_stmt_list (s: stmt list) (in_decls: bool) =
  ...
  | Decl d :: r when in_decls ->
      group (p_declaration_and_initialization d ^^ semi ^^ hardline ^^ p_stmt_list r true)
  | Decl d :: r when not in_decls ->
      group (lbrace ^^ hardline ^^ p_declaration_and_initialization d ^^ semi ^^ hardline ^^ p_stmt_list r false ^^ rbrace)
  | f :: r -> 
...
and p_stmt (s: stmt) =
  ...
  match s with
  | Compound stmts when !Options.cc = "gcc-c89" ->
  ...
  | For (decl, e2, e3, stmt) when !Options.cc = "gcc-c89" ->
      group (lbrace ^^ hardline ^^ p_declaration decl ^^ semi ^^ hardline ^^
	     string "for" ^/^ lparen ^^ nest 2 (
             p_initialization decl ^^ semi ^^ break1 ^^
  ...
  | Decl d ->
      group (p_declaration_and_initialization d ^^ semi)
...
let p_decl_or_function (df: declaration_or_function) =
  ...
  | Function (inline, d, stmt) ->
      ...
      inline ^^ group (p_declaration_and_initialization d) ^/^ p_stmt stmt
