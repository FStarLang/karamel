...
let remove_trivial_matches = object (self)
  ...
  method! ematch () _ e branches =
    ...
    | EBool true, [ [], { node = PBool true; _ }, b1; [ v ], { node = PBound 0; _ }, _ ] when !(v.node.mark) = 0 ->
        (* An if-then-else is generated as:
         *   match e with true -> ... | uu____???? -> ...
         * where uu____???? is unused. *)
        let _, b1 = open_binder v b1 in
        let b1 = self#visit () b1 in
        b1.node
    | EBool false, [ [], { node = PBool true; _ }, _; [ v ], { node = PBound 0; _ }, b2 ] when !(v.node.mark) = 0 ->
