...
let lstring : eqtype = list char
let sstring : eqtype = Seq.seq char
...
*)
type key =
| Key: (s: lstring {not (List.Tot.mem double_quote s)}) -> key

type json_schema =
| String
| Object of (l: list (key * json_schema) {List.Tot.noRepeats (List.Tot.map fst l)} )

type ufalse : Type 'ua = | UFALSE : False -> ufalse

let rec object_as_type (j: json_schema) (as_type: (j': json_schema {j' << j}) -> Tot Type) (l: list (key * json_schema) {l << j}) (s: key) : Tot Type =
match l with
| [] -> ufalse
...
let rec object_as_type_append_cons
  (j: json_schema)
  (as_type: (j': json_schema {j' << j}) -> Tot Type)
...
let object_as_type' (j: json_schema) (as_type: (j': json_schema {j' << j}) -> Tot Type) (l: list (key * json_schema) {l << j}) (s: key) : Tot Type =
  match List.Tot.assoc s l with
  | None -> ufalse
  | Some j' -> List.Tot.assoc_precedes s l j'; as_type j'

let rec object_as_type_assoc  (j: json_schema) (as_type: (j': json_schema {j' << j}) -> Tot Type) (l: list (key * json_schema) {l << j}) (s: key) :
  Lemma (requires True)
  (ensures (object_as_type' j as_type l s == object_as_type j as_type l s))
  (decreases l)
= match l with
  | [] -> ()
  | (s', _) :: q -> if s = s' then () else object_as_type_assoc j as_type q s


let rec as_gtype (j: json_schema) : Tot Type =
  match j with
  | String -> (s: sstring { ~ (Seq.mem double_quote s) } )
  | Object l ->
...
let as_gtype_string: squash (as_gtype String == (s: sstring { ~ (Seq.mem double_quote s ) } )) = ()

let gprint_string (s: sstring { ~ (Seq.mem double_quote s) } ) : GTot sstring =
  Seq.cons double_quote (Seq.snoc s double_quote)

let object_rec_prereq
  (as_type: json_schema -> Tot Type)
  (j: json_schema)
  (l: list (key * json_schema) {List.Tot.noRepeats (List.Tot.map fst l) /\ l << j})
  (data: DependentMap.t (s: key {List.Tot.mem s (List.Tot.map fst l)}) (object_as_type j as_type l))
...
: Lemma
  (ensures
    ...
    /\ object_as_type j as_type l s == as_type j'
    /\ j' << l /\ j' << j
...
= match l' with
  | [] -> Seq.createEmpty
  | ((s, j')::q) ->
    object_rec_prereq as_gtype j l data l' s j' q;
    let (data' : as_gtype j') =
	...
	Seq.createEmpty
    in
    Seq.append (gprint_string (Seq.seq_of_list (Key?.s s))) (Seq.cons colon (Seq.append objprint suff))

let rec gprint (j: json_schema) (data: as_gtype j) : GTot sstring =
  match j with
...
let rec gparse_whitespace_append
  (s_white: sstring { forall (i: nat { i < Seq.length s_white } ) . is_whitespace (Seq.index s_white i) } )
  (s: sstring)
: Lemma
  (requires True)
...
= if Seq.length s_white = 0
  then begin
    ...
    in
    let s_white: (s_white: sstring {Seq.length s_white > 0 /\ ( forall (i: nat { i < Seq.length s_white } ) . is_whitespace (Seq.index s_white i)) } ) = s_white in
...
let gparse_whitespace_not_whitespace
  (s: sstring { Seq.length s > 0 /\ ~ (is_whitespace (Seq.index s 0)) } )
: Lemma
...
let rec gparse_string_contents_append_cons
  (accu: sstring)
  (left: sstring {~ (Seq.mem double_quote left) } )
  (right: sstring)
: Lemma
  (requires True)
...
let gparse_string (s: sstring): GTot (option (sstring * sstring)) =
...
let gparse_string_append_cons_append_cons
  (s_white: sstring { forall (i: nat { i < Seq.length s_white } ) . is_whitespace (Seq.index s_white i) } )
  (s_contents: sstring { ~ (Seq.mem double_quote s_contents) } )
  (s_tail: sstring)
: Lemma
...
= match against with
  | [] -> Some parsee
  | hd_against :: tl_against ->
    if Seq.length parsee = 0
    then None
    else if hd_against <> Seq.head parsee
    then None
    else gparse_exact_string_contents (tl_against) (Seq.tail parsee)

let rec gparse_exact_string_contents_append
  (against: lstring)
  (tail: sstring)
: Lemma
  (ensures (gparse_exact_string_contents against (Seq.append (Seq.seq_of_list against) tail) == Some tail))
= match against with
  | [] ->
    Seq.append_empty_l tail
  | hd_against :: tl_against ->
    let _ = assert (Seq.seq_of_list against == Seq.cons hd_against (Seq.seq_of_list tl_against)) in
    let _ = Seq.append_cons hd_against (Seq.seq_of_list tl_against) tail in
    let _ = Seq.lemma_tl hd_against (Seq.seq_of_list tl_against) in
    let _ = Seq.lemma_tail_append (Seq.seq_of_list against) tail in
    gparse_exact_string_contents_append tl_against tail
...
= match against with
  | [] -> ()
  | _ :: tl_against -> length_gparse_exact_string_contents (tl_against) (Seq.tail parsee) tail

let gparse_exact_string
  (against: lstring { ~ (List.Tot.mem double_quote against) })
  (parsee: sstring)
: GTot (option sstring)
...
let gparse_exact_string_gprint_string
  (white: sstring { forall (i: nat { i < Seq.length white } ) . is_whitespace (Seq.index white i) } )
  (against: lstring {~ (List.Tot.mem double_quote against) } )
  (tail: sstring)
: Lemma
  (ensures (gparse_exact_string against (Seq.append white (Seq.append (gprint_string (Seq.seq_of_list against)) tail)) == Some tail))
= let _ : squash (Seq.append (gprint_string (Seq.seq_of_list against)) tail == Seq.cons double_quote (Seq.append (Seq.seq_of_list against) (Seq.cons double_quote tail))) =
    Seq.append_cons double_quote (Seq.snoc (Seq.seq_of_list against) double_quote) tail;
    Seq.append_cons_snoc (Seq.seq_of_list against) double_quote tail
  in
  gparse_exact_char_append_cons white double_quote (Seq.append (Seq.seq_of_list against) (Seq.cons double_quote tail));
...
let rec gparse_object
  (j: json_schema)
  (gparse: (j': json_schema {j' << j}) -> (data: as_gtype j') -> (s: sstring) -> GTot (option (as_gtype j' * (s': sstring {Seq.length s' < Seq.length s}))))
  (l: list (key * json_schema) {List.Tot.noRepeats (List.Tot.map fst l) /\ l << j})
  (data: DependentMap.t (s: key {List.Tot.mem s (List.Tot.map fst l)}) (object_as_type j as_gtype l))
  (l': list (key * json_schema) {List.Tot.strict_prefix_of l' l \/ l' == l})
  (s: sstring)
: Ghost (option (DependentMap.t (s: key {List.Tot.mem s (List.Tot.map fst l)}) (object_as_type j as_gtype l) * (s': sstring {Seq.length s' <= Seq.length s} )))
  (requires True)
  (ensures (fun _ -> True))
  (decreases l')
= match l' with
  ...
  | ((s', j') :: q) ->
	...
	| Some (new_data', s3) ->
	  ...
	  | Some s4 ->
	    ...
	    | None -> None
	    | Some (new_data_, s5) ->
	      let (s6: sstring {Seq.length s6 <= Seq.length s}) = s5 in
	      Some (new_data_, s6)
	    end
	  end else
	    Some (new_data, s3)
...
let rec gparse
  ...
  (data: as_gtype j)
  (s: sstring)
: Ghost (option (as_gtype j * (s': sstring {Seq.length s' < Seq.length s})))
  (requires True)
  (ensures (fun _ -> True))
  (decreases j)
= match j with
  ...
  | Object l ->
    ...
    | Some s1 ->
      ...
      let gparse_rec (j': json_schema {j' << j}) (data: as_gtype j') (s: sstring) : GTot (option (as_gtype j' * (s': sstring {Seq.length s' < Seq.length s}))) = gparse j' data s in
...
let gparse_object_cons_nil
  ...
  (s: sstring)
  (s1: sstring { Seq.length s1 < Seq.length s /\ gparse_exact_string (Key?.s s') s == Some s1 } )
  (s2: sstring { Seq.length s2 < Seq.length s1 /\ gparse_exact_char colon s1 == Some s2 } )
  (data': as_gtype j' { data' == DependentMap.sel data s' } )
  (new_data': as_gtype j')
  (s3: sstring { Seq.length s3 < Seq.length s2 /\ gparse j' data' s2 == Some (new_data', s3) } )
...
let gparse_object_gprint_object_aux_cons_nil
  ...
  (s: sstring {s == Seq.append (gprint_object j gprint l data l') tail } )
  (s2: sstring { Seq.length s2 < Seq.length s /\ gparse_exact_string (Key?.s s') s == Some s2 } )
  (s3: sstring { Seq.length s3 < Seq.length s2 /\ gparse_exact_char colon s2 == Some s3 } )
  (data': as_gtype j' { DependentMap.sel data s' == data' } )
  (data0': as_gtype j' { DependentMap.sel data0 s' == data0' } )
  (s4: sstring { Seq.length s4 < Seq.length s3  /\ gparse j' data0' s3 == Some (data', s4) /\ s4 == Seq.append Seq.createEmpty tail } )
...
= object_rec_prereq as_gtype j l data l' s' j' q;
  ...
  let _ : squash (gparse_object j gparse l data0 l' s == Some (new_data0, s4)) =
    ...
    let (s2: sstring { Seq.length s2 < Seq.length s /\ gparse_exact_string (Key?.s s') s == Some s2 } ) = s2 in
    let (s3: sstring { Seq.length s3 < Seq.length s2 /\ gparse_exact_char colon s2 == Some s3 } ) = s3 in
    let (data0': as_gtype j' {data0' == DependentMap.sel data0 s' } ) = data0' in
    let (data': as_gtype j') = data' in
    let (s4: sstring { Seq.length s4 < Seq.length s3 /\ gparse j' data0' s3 == Some (data', s4) } ) = s4 in
...
let gparse_object_cons_cons
  ...
  (s3: sstring { Seq.length s3 < Seq.length s2 /\ gparse j' data' s2 == Some (new_data', s3) } )
  (new_data: DependentMap.t (s: key {List.Tot.mem s (List.Tot.map fst l)}) (object_as_type j as_gtype l) { new_data == DependentMap.upd data s' new_data' } )
  (s4: sstring { Seq.length s4 < Seq.length s3 /\ gparse_exact_char comma s3 == Some s4 } )
  (data_: DependentMap.t (s: key {List.Tot.mem s (List.Tot.map fst l)}) (object_as_type j as_gtype l))
  (s5: sstring { Seq.length s5 <= Seq.length s4 /\ gparse_object j gparse l new_data q s4 == Some (data_, s5) } )
: Lemma
...
let gparse_object_gprint_object_aux_cons_cons
  ...
  (data': as_gtype j' { DependentMap.sel data s' == data' } )
  (data0': as_gtype j' { DependentMap.sel data0 s' == data0' } )
  (s4: sstring { Seq.length s4 < Seq.length s3  /\ gparse j' data0' s3 == Some (data', s4) /\ s4 == Seq.append (Seq.cons comma (gprint_object j gprint l data q)) tail } )
  (new_l_left: list (key * json_schema) { l == List.Tot.append new_l_left q /\ Cons? q } )
  (new_data0: DependentMap.t (s: key {List.Tot.mem s (List.Tot.map fst l)}) (object_as_type j as_gtype l) {new_data0 == DependentMap.upd data0 s' data' })
  (new_hsel: (
    (k: key {List.Tot.mem k (List.Tot.map fst l) /\ List.Tot.mem k (List.Tot.map fst new_l_left)}) ->
...
let gparse_object_gprint_object_aux_cons
  ...
  (data: DependentMap.t (s: key {List.Tot.mem s (List.Tot.map fst l)}) (object_as_type j as_gtype l))  
  (tail: sstring)
  (ihgparse: (j': json_schema {j' << j}) -> (data0': as_gtype j') -> (data': as_gtype j') -> (tail': sstring) ->
...
= object_rec_prereq as_gtype j l data l' s' j' q;
  ...
  let pkey = gprint_string (Seq.seq_of_list (Key?.s s')) in
